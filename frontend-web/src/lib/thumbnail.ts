...
export function buildMarketingThumbnailSvg(spec: ThumbnailSpec): string {
  ...
  const titleSize = isCentered
    ? Math.round(width * 0.25)
    : isWide
      ? 72
      : Math.round(width * (isTall ? 0.14 : 0.12));
  const subSize = isCentered
    ? Math.round(width * 0.12)
    : isWide
      ? 30
      : Math.round(width * (isTall ? 0.07 : 0.06));
  ...
  const footerLeft = spec.copy.footerLeft
    ? escapeXml(spec.copy.footerLeft)
    : "";
  const footerRight = spec.copy.footerRight
    ? escapeXml(spec.copy.footerRight)
    : "";
  ...
  const titleY = isCentered
    ...
    : Math.round(pad + (isWide ? 140 : isTall ? 120 : 96));
  const subY = isCentered
    ? Math.round(height * 0.65)
    : titleY + Math.round(titleSize * (isTall ? 1.4 : 0.78));
  ...
  const bulletTextSize = isWide
    ? 24
    : Math.round(width * (isTall ? 0.055 : 0.05));
    ...
    <g>
      ...
      ${(() => {
        ...
        if (match && match[1] === "30") {
          ...
          const offset =
            rest.length * smallSize * 1.0 + num.length * bigSize * 0.55 + 16;
