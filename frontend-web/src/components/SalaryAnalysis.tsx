...
export function SalaryAnalysis({ onUnlock }: SalaryAnalysisProps) {
  ...
  return (
    <Card className="p-6 bg-white border-white/60 shadow-xl shadow-black/5 flex flex-col h-[320px] relative overflow-hidden group cursor-pointer" onClick={onUnlock}>
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-bold text-gray-900 flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div>
          身价评估 (年薪/万)
        </h3>
        <div className="bg-black/5 px-2 py-1 rounded text-[10px] font-bold text-gray-500">PRO FEATURE</div>
      </div>
      ...
      <div className="flex-1 w-full min-h-0 relative">
        ...
        <div className="absolute inset-0 z-10 backdrop-blur-[6px] bg-white/30 flex flex-col items-center justify-center transition-all group-hover:backdrop-blur-[4px]">
            ...
            <p className="text-sm font-bold text-gray-900">解锁查看你的真实身价</p>
          ...
          <BarChart data={data} margin={{ top: 20, right: 30, left: 0, bottom: 0 }}>
            ...
            <Bar dataKey="salary" radius={[4, 4, 0, 0]} barSize={40}>
              {data.map((entry, index) => (
                <Cell key={`cell-${index}`} fill={entry.name === 'You' ? '#000000' : '#e5e7eb'} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
